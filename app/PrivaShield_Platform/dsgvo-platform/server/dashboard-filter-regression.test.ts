...
describe("dashboard filter route regression guards", () => {
  ...
  it("keeps all dashboard links on supported module routes", () => {
    ...
    for (const href of dashboardLinks) {
      expect(href.startsWith("/aufgaben?") || href.startsWith("/pdca?") || href.startsWith("/datenpannen?") || href.startsWith("/dsfa?") || href.startsWith("/avv?") || href.startsWith("/tom?") || href.startsWith("/vvt?") || href.startsWith("/dsr?") || href.startsWith("/loeschkonzept?") || href.startsWith("/ki-compliance?") || href.startsWith("/audits?")).toBe(true);
    }
  });
